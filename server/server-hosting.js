...
const uploadsDir = path.join(__dirname, 'uploads-tokokosmetik-ariani');
console.log('Initializing uploads directory:', uploadsDir);
...
const db = mysql.createConnection({
  host: 'localhost',
  user: 'isad8273_lily_mily_kosmetik',
  password: 'isad8273_lily_mily_kosmetik',
  database: 'isad8273_lily_mily_kosmetik',
...
// Error handling middleware for brand logo upload
...
app.post('/api/brands', handleLogoUpload, (req, res) => {
  const { name, description } = req.body;
...
app.put('/api/brands/:id', handleLogoUpload, (req, res) => {
  const { id } = req.params;
  const { name, description } = req.body;
...
app.post('/api/products', handleUpload, (req, res) => {
  try {
    const { name, description, price, online_store_link, brand_id, category_id } = req.body;
    const image = req.file ? req.file.filename : null;
    
    console.log('Uploading product with image:', image);
    ...
    db.query(
      ...
      (err, results) => {
        ...
        console.log('Product saved successfully with ID:', results.insertId);
        res.json({ id: results.insertId, message: 'Product created successfully', image: image });
      }
    );
  } catch (error) {
...
app.put('/api/products/:id', handleUpload, (req, res) => {
  const { id } = req.params;
  const { name, description, price, online_store_link, brand_id, category_id } = req.body;
...
app.post('/api/upload', upload.single('file'), (req, res) => {
  try {
    ...
    console.log('File uploaded via /api/upload:', req.file.filename);
    console.log('File path:', req.file.path);
    res.json({ 
